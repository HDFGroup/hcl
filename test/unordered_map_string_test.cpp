...
int main(int argc, char *argv[]) {
    ...
    if (is_server) {
        shm_map = new hcl::unordered_map<KeyType, std::string, CharAllocator, MappedUnitString>("SHARED_UM");
    }
    MPI_Barrier(MPI_COMM_WORLD);
    if (!is_server) {
        shm_map = new hcl::unordered_map<KeyType, std::string, CharAllocator, MappedUnitString>("SHARED_UM");
    ...
    remote_map = new hcl::unordered_map<KeyType, std::string, CharAllocator, MappedUnitString>("REMOTE_UM");
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    double llocal_map_throughput =
            num_request / llocal_map_timer.getElapsedTime() * 1000 * size_of_elem * size_of_request / 1024 / 1024;
    ...
    double llocal_get_map_throughput =
            num_request / llocal_get_map_timer.getElapsedTime() * 1000 * size_of_elem * size_of_request / 1024 / 1024;
    ...
    for (int i = 0; i < num_request; i++) {
        ...
        auto result = shm_map->Put(key, shared_vals);
        local_map_timer.pauseTime();
        assert(result);
    }
    double local_map_throughput =
            num_request / local_map_timer.getElapsedTime() * 1000 * size_of_elem * size_of_request / 1024 / 1024;
    ...
    double local_get_map_throughput =
            num_request / local_get_map_timer.getElapsedTime() * 1000 * size_of_elem * size_of_request / 1024 / 1024;
    ...
    for (int i = 0; i < num_request; i++) {
        ...
        auto result = remote_map->Put(key, shared_vals);
        remote_map_timer.pauseTime();
        assert(result);
    }
    double remote_map_throughput =
            num_request / remote_map_timer.getElapsedTime() * 1000 * size_of_elem * size_of_request / 1024 / 1024;
    ...
    double remote_get_map_throughput =
            num_request / remote_get_map_timer.getElapsedTime() * 1000 * size_of_elem * size_of_request / 1024 / 1024;
