...
int main (int argc,char* argv[])
{
    ...
    hcl::multimap<KeyType,MyType> *multimap;
    if (is_server) {
        multimap = new hcl::multimap<KeyType,MyType>();
    }
    MPI_Barrier(MPI_COMM_WORLD);
    if (!is_server) {
        multimap = new hcl::multimap<KeyType,MyType>();
    ...
    std::multimap<KeyType,MyType> lmultimap=std::multimap<KeyType,MyType>();
    ...
    //     int a;
    //     std::function<std::pair<bool,int>(KeyType&,MyType&,std::string,int)> putFunc(std::bind(&hcl::multimap<KeyType,std::array<int,
    ...
    if (!is_server) {
        ...
        for(int i=0;i<num_request;i++){
            ...
            lmultimap.insert(std::pair<KeyType, MyType>(KeyType(val), my_vals));
